...
try:
    ...
    # try without temp_uploads just in case
    import os
    files = [f for f in os.listdir("temp_uploads") if "GB50243" in f]
    if files:
        pdf_path = "temp_uploads/" + files[0]
    else:
        sys.exit(1)
...
for idx, tbl in enumerate(tables[:2]):
    ...
    for r in rows[:2]:  # first 2 rows
        ...
        for c in cells[:2]: # first 2 cells
            ...
            for k in kids:
                print("   Kid JSON:", json.dumps(k, ensure_ascii=False))
